...
priLikDAlpha<-function(y,y0,dalpha,dgamma,pre.dalpha,pre.dgamma,xabeta,sigma,betaSD){
  ...
  priDAlpha<-sum(ifelse(dtnorm(dalpha, low=1, mean = xabeta, sd = betaSD,log=T)==Inf,0,dtnorm(dalpha, low=1, mean = xabeta, sd = betaSD,log=T)))#(dtnorm(dalpha, low=1, mean = xabeta, sd = betaSD,log=T))#dlnorm(dalpha,meanlog =xabeta,sdlog =betaSD,log=T)#dunif(dalpha,exp(xabeta)-(sqrt(3)*betaSD),max =exp(xabeta)+(sqrt(3)*betaSD),log=T)#
...
pos_DAlpGamMu0<-function(y,y0,dalpha,dgamma,XAbeta,XGbeta,sigma,betaASD,betaGSD){
  ...
  for(i in 1:7){
    ...
    dalpha.s<-rtruncnorm(1,mean = dalpha[i],sd =1,a=1,b=2)
    ...
    r = exp(llikp - llik
            -log(dtruncnorm(dalpha.s, mean=dalpha[i], sd=1, a=1,b=2))
            +log(dtruncnorm(dalpha[i], mean=dalpha.s, sd=1, a=1,b=2)))
    accept<-min(1,r)
    z<-runif(1)
    if(z<accept){
      dalpha[i] = dalpha.s
    ...
    dgamma.s<-rtruncnorm(1,mean = dgamma[i],sd = 1, a=1,b=2)
    ...
    r = exp(llikpG - llikG
            -log(dtruncnorm(dgamma.s, mean=dgamma[i], sd=1, a=1,b=2))
            +log(dtruncnorm(dgamma[i], mean=dgamma.s, sd=1, a=1,b=2)))
    accept<-min(1,r)
    z<-runif(1)
    if(z<accept){
      dgamma[i] = dgamma.s
...
posSigma2<-function(y,Gamma,Alpha,Sigma2){
  ...
  Sigma2.s<-abs(rnorm(1,Sigma2,sd=0.0005))#rtruncnorm(1,mean =Sigma2,sd = 0.00005, a=0)#abs(rnorm(1,Sigma2,sd=0.005))#Sigma2.s=0.0002
