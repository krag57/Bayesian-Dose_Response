...
pos_DAlpGamMu0<-function(y,y0,mu0,dalpha,dgamma,XAbeta,XGbeta,sigma,betaASD,betaGSD,pMu0,pSigma0){
  ...
  for(i in 1:7){
    ...
    dalpha.s<-rtruncnorm(1,mean = dalpha[i],sd = .3,a=0)
    ...
    r = exp(llikp - llik
            -log(dtruncnorm(dalpha.s, mean=dalpha[i], sd=.3, a=0))
            +log(dtruncnorm(dalpha[i], mean=dalpha.s, sd=.3, a=0)))
    accept<-min(1,r)
    z<-runif(1)
    if(z<accept){
      dalpha[i] = dalpha.s
    ...
    dgamma.s<-rtruncnorm(1,mean = dgamma[i],sd = .3, a=0)
    ...
    r = exp(llikpG - llikG
            -log(dtruncnorm(dgamma.s, mean=dgamma[i], sd=.3, a=0))
            +log(dtruncnorm(dgamma[i], mean=dgamma.s, sd=.3, a=0)))
    accept<-min(1,r)
    z<-runif(1)
    if(z<accept){
      dgamma[i] = dgamma.s
