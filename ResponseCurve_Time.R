...
SB590885_res=response[response$Small.Molecule.Name=='SB590885' &
                        response$Drug.Concentration..uM %in% c(0.3160,1.0000,3.1600),
                      c(1,6)]
DataMatrix<-matrix(SB590885_res[,2],ncol=9,byrow = T)
...
for (s in seq(0,1,0.01)){
 ...
 FinalData<-rbind(FinalData,M_kill[,1])
}


plot(PVol,FinalData[1,],col="blue",t="l",lwd=2)
lines(PVol,FinalData[1,],col="black",t="l",lwd=2)
...
# 
# toc
#FinalData50<-FinalData
#FinalData100<-FinalData
#FinalDataAll<-FinalData
matplot(PVol,t(FinalData),col = 1:50,type = "l")
line
