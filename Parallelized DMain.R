...
for (p in 2:M){
  ...
  capture.output(baa<-bridge(Xbind,matrix(log(resDAlpha[p,,]), ncol = 1),RJ=F,ab=c(1, .5)), file='NUL')
  beta<-colMeans(baa$beta[-(1:500),])
  s2a=(sqrt(mean(baa$s2)))
  
  capture.output(bag<-bridge(Xbind,matrix(log(resDGamma[p,,]), ncol = 1),RJ=F,ab=c(1,.5)), file='NUL')
