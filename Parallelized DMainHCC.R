...
for (p in 2:M){
  ...
  capture.output(baa<-bridge(Xbind,matrix(log(resDAlpha[p,,]), ncol = 1),RJ=F,ab=c(2, .2)), file='NUL')
  beta<-colMeans(baa$beta[-(1:500),])
  s2a=(sqrt(mean(baa$s2)))
  
  capture.output(bag<-bridge(Xbind,matrix(log(resDGamma[p,,]), ncol = 1),RJ=F,ab=c(2, .2)), file='NUL')
  beta_g<-colMeans(bag$beta[-(1:500),])
  s2g=(sqrt(mean(bag$s2)))
  
  
}
save.image(paste0("/work/statgrads/krag57/DSim",j,"/SimData.RData"))
stopImplicitCluster()
...
# traceplot(as.mcmc(alphaGamma(resAlpha[(M/2):M,,],6,1)))
